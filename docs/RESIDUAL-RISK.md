...
| RR-02 | **Signing uses a pure-Python Ed25519 reference** (`scripts/aers_assure/_ed25519.py`). | Dependency-free offline operation; no audited crypto lib is installed. | Verification is RFC-8032 correct (accepts genuine Ed25519 signatures, rejects forgeries — see `test_verifier.py`); production swaps in an audited signing library on the verifier side, and this module verifies those signatures unchanged. The repo holds no production private key. Owner: security. | production rollout |
